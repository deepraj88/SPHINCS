 ...
 */
void thash(unsigned char out[SPX_N], const unsigned char in[35*SPX_N], unsigned int inblocks,
           const unsigned char *pub_seed, uint32_t addr[8])
{
    unsigned char buf[SPX_ADDR_BYTES + 35*SPX_N]={0};
    unsigned char outbuf[32];
    unsigned char buf_tmp[64]={0};
    int loop;
    /*static count_max = 0;
