...
main()
{
...
#ifdef READFILE
    sprintf(fn_rsp, "../../../../../NIST-PQ-Submission-SPHINCS-20190329/KAT/sphincs-haraka-128f-simple/PQCsignKAT_64.rsp");
    ...
    do {
        ...
        if ( !ReadHex(fp_rsp, seed, 48, "seed = ") ) {
            printf("ERR: unable to read 'seed' from <%s>\n", fn_rsp);
        ...
        if ( FindMarker(fp_rsp, "mlen = ") )
            ...
            printf("ERR: unable to read 'mlen' from <%s>\n", fn_rsp);
            return KAT_DATA_ERROR;
        }

        if ( !ReadHex(fp_rsp, m, (int)mlen, "msg = ") ) {
            printf("ERR: unable to read 'msg' from <%s>\n", fn_rsp);
            return KAT_DATA_ERROR;
        }
#endif
        ...
        if ( !ReadHex(fp_rsp, pk_rsp, CRYPTO_PUBLICKEYBYTES, "pk = ") ) {
            printf("ERR: unable to read 'pk' from <%s>\n", fn_rsp);
            return KAT_DATA_ERROR;
        }
        if ( !ReadHex(fp_rsp, sk_rsp, CRYPTO_SECRETKEYBYTES, "sk = ") ) {
            printf("ERR: unable to read 'sk' from <%s>\n", fn_rsp);
            return KAT_DATA_ERROR;
        }
...
//	    printf("ERR: sk is different from <%s>\n", fn_rsp);
//	    //return KAT_VERIFICATION_ERROR;
//	}
#endif
        ...
        if ( !ReadHex(fp_rsp, sm_rsp, smlen_rsp, "sm = ") ) {
            printf("ERR: unable to read 'sm' from <%s>\n", fn_rsp);
            return KAT_DATA_ERROR;
        }
