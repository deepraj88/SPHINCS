...
{
    unsigned char stack[(/*tree_height*/4 + 1)*SPX_N];
    unsigned int heights[/*tree_height*/4 + 1];
    unsigned int offset = 0;
    uint32_t idx;
    uint32_t tree_idx;
    int loop;

    for (idx = 0; idx < 16/*(uint32_t)(1 << tree_height)*/; idx++) {
...
void treehash_fors(unsigned char *root, unsigned char auth_path[SPX_N],
              ...
              uint32_t tree_addr[8])
{
    unsigned char stack[(/*tree_height*/10 + 1)*SPX_N];
    unsigned int heights[/*tree_height*/10 + 1];
    unsigned int offset = 0;
    uint32_t idx;
    uint32_t tree_idx;
    int loop;

    for (idx = 0; idx < 1024/*(uint32_t)(1 << tree_height)*/; idx++) {
        /* Add the next leaf node to the stack. */
